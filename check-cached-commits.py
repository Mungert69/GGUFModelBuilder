...
def find_huggingface_model(model_name):
    ...
    if models:
        # Return the first match along with its model ID and base model information
        model_info = models[0]
        return {
            "model_id": model_info.modelId,
            "base_model": model_info.config.get("base_model", None)  # Extract base model if available
        }
    return None
